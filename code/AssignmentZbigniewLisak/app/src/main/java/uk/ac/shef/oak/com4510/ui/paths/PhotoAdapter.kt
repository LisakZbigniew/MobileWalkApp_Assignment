...
class PhotoAdapter : RecyclerView.Adapter<PhotoAdapter.ViewHolder> {
    ...
    constructor(items: List<Photo>) : super() {
        PhotoAdapter.items = items as MutableList<Photo>
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        val holder: PhotoAdapter.ViewHolder = ViewHolder(v)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (Path(items[position].thumbnailUri).exists()) {
            ...
            scope.launch(Dispatchers.Main) {
                //would sometimes throw out of bounds exception hence the check,
                //the items might be changed before the execution reaches here, but im not sure why its happening
